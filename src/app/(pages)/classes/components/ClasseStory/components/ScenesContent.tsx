...
const ScenesContent = ( { image_path, title, body, position }:IProps ) => {
  ...
  return (
    <div className={`flex ${position === 'right' ? 'justify-end' : 'justify-start'} gap-4`}>
      <div className="flex flex-col items-end w-full">
        <h5 className="pr-2 text-end font-semibold text-lg text-amber-400">{title}</h5>
        <CompDivisor />
      ...
      {modalIsOpened &&(
        <div className="fixed z-10 w-screen h-screen bg-slate-900/50 top-0 left-0" id={'sceneBackdrop'} onClick={(event) => handleBackdrop(event)}>
          <div className="absolute w-[600px] max-w-[95vw] h-[700px] bg-slate-700 top-[50%] left-[50%] rounded-md transform translate-x-[-50%] translate-y-[-50%] z-20 p-3 pt-10 flex flex-col items-center">
